...
bool allocate_states(DFA dfa, StatesInt new_amount) ; // Allocate the memory required to the
...
void add_token_to_DFA(DFA, char*, TokenType, StatesInt); // Set up the transitions for a token in the transition table
// and the state array

void add_alnum_token_to_DFA(DFA, char*, TokenType, StatesInt); // Set up the transitions for a token in the transition table
